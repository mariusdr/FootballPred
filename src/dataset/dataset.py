...
class FootballDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # get teams
        team_home = self.teams.loc[self.teams['team_api_id'] ==
                                   match["home_team_api_id"]]
        team_away = self.teams.loc[self.teams['team_api_id'] ==
        ...
        X = [
            encoded_team_home, encoded_team_away, encoded_players_home,
            encoded_players_away, trend_home, trend_away
        ]
    ...
    def encode_player(self, player):
        ...
        for category in category_values:
            vals.append(
                self.one_hot_category(player[category],
                                      self.players[category].unique()))
    ...
    def encode_team(self, team):
        ...
        for category in category_values:
            vals.append(
                self.one_hot_category(team[category],
                                      self.teams[category].unique()))
    ...
    def one_hot_category(self, value, categories):
        return self.one_hot_value_int(
            np.where(categories == value)[0], 0,
            len(categories) - 1)
